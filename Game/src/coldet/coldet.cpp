...
bool SphereSegmentCollision(float center[3], float radius,
                        float origin[3], float direction[3], float limit[3],
                        float point[3])
{
  Vector3D& C=*((Vector3D*)center);
  Vector3D& O=*((Vector3D*)origin);
