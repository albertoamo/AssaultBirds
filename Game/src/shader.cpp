...
bool Shader::load(const std::string& vsf, const std::string& psf)
{
	...
	#ifdef _DEBUG
		std::cout << " * Shader: Vertex: " << vsf  << std::endl << "\tPixel: " << psf << std::endl;
	#endif
...
bool Shader::load(const std::string& vsf, const std::string& psf, const std::string& macros)
{
	assert(compiled == false);
	assert(glGetError() == GL_NO_ERROR);
...
Shader* Shader::Load(const char* vsf, const char* psf, const char* macros)
{
...
bool Shader::createShaderObject(unsigned int type, GLuint& handle, const std::string& code)
{
	...
	//std::cout << prefix << "  PAINLESS SHIT "<< std::endl;
